...
addResourcePath("img_folder","websiteicons")

resource_images <- c(
  "img_folder/ComunityEvents.png",
  "img_folder/Meals.png",
  "img_folder/Healthcare.png",
  "img_folder/TechnologyWifi.png",
  "img_folder/Meals.png",
  "img_folder/ComunityEvents.png",
  "img_folder/Shelter.png",
  "img_folder/Healthcare.png",
  "img_folder/Shelter.png",
  "img_folder/Shelter.png",
  "img_folder/ComunityEvents.png",
...
ui <- fluidPage(
    ...
    sidebarLayout(
              ...
              mainPanel(
                    ...
                    p(),
                    h3("Map for Directions", style = "color: #a1bb96;"),
                    leafletOutput("mymap")
                )
        )
      ),
...
server <- function(input, output, session) {
  ...
  output$data <- DT::renderDataTable({ 
   ...
   DT::datatable( out,list( pageLength=3 ), selection="none", escape = FALSE )
  })
  
  output$data_events <- DT::renderDataTable({ 
    out2<-Dat2
    DT::datatable( out2,list( pageLength=3 ),selection="none",escape = FALSE )
  })
  ...
  Map_Dat<- reactive({
    ...
    ## caso nada selecionada busca default
    else(
  ...
  pal <- colorNumeric(c("red", "green", "blue"), 50:500)
  
  output$mymap <- renderLeaflet({
        leaflet(Map_Dat()) %>% setView(-86.1581, 39.7684, zoom = 10) %>% 
        addTiles() %>%  # Add default OpenStreetMap map tiles
        addMarkers( data=Map_Dat(),lng= ~lon,lat= ~lat, popup=~paste( Resource_name, "<br>", Google_map )) %>%
